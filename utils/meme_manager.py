...
class MemeManager:
    ...
    def __init__(self, store_file: Path, max_per_tag: int = 200):
        ...
        self.image_entries: dict[str, dict[str, str]] = {}
        self._load()

    def add_meme(self, image_id: str, tag: str, image_entry: dict[str, str] | None = None) -> tuple[bool, str]:
        ...
        if image_entry is not None:
            self.image_entries[normalized_id] = {
                "url": str(image_entry.get("url") or "").strip(),
                "local_path": str(image_entry.get("local_path") or "").strip(),
                "thumb_path": str(image_entry.get("thumb_path") or "").strip(),
                "keyword": str(image_entry.get("keyword") or "").strip(),
                "content": str(image_entry.get("content") or "").strip(),
            }
    ...
    def get_image_entry(self, image_id: str) -> dict[str, str] | None:
        normalized_id = str(image_id or "").strip()
        if not normalized_id:
            return None
        entry = self.image_entries.get(normalized_id)
        if not isinstance(entry, dict):
            return None
        return {
            "url": str(entry.get("url") or "").strip(),
            "local_path": str(entry.get("local_path") or "").strip(),
            "thumb_path": str(entry.get("thumb_path") or "").strip(),
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raw_entries = data.get("image_entries", {}) if isinstance(data, dict) else {}
            if isinstance(raw_entries, dict):
                loaded_entries: dict[str, dict[str, str]] = {}
                for raw_id, raw_entry in raw_entries.items():
                    image_id = str(raw_id or "").strip()
                    if not image_id or not isinstance(raw_entry, dict):
                        continue
                    loaded_entries[image_id] = {
                        "url": str(raw_entry.get("url") or "").strip(),
                        "local_path": str(raw_entry.get("local_path") or "").strip(),
                        "thumb_path": str(raw_entry.get("thumb_path") or "").strip(),
                        "keyword": str(raw_entry.get("keyword") or "").strip(),
                        "content": str(raw_entry.get("content") or "").strip(),
                    }
                self.image_entries = loaded_entries
    ...
    def _save(self) -> None:
        try:
            payload = {
                "memes_by_tag": self.memes_by_tag,
